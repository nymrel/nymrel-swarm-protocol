...
export class TwoSeatProtocol {
   ...
   */
  async initMission(params: InitMissionParams): Promise<MissionRecord> {
    return await this.lockManager.withLock(`mission_${params.mission_id}`, async () => {
      ...
      const fencingToken = await this.fencingClock.incrementGeneration(
        `mission_${params.mission_id}`,
        params.mission_owner_seat_id
      );
  ...
  async getMission(missionId: string): Promise<MissionRecord | null> {
    ...
    try {
      ...
      return JSON.parse(content) as MissionRecord;
    } catch {
      return null;
    }
  }
   ...
   */
  async requestHandover(missionId: string, packet: HandoverPacket): Promise<HandoverResult> {
    return await this.lockManager.withLock(`mission_${missionId}`, async () => {
      ...
      if (mission.mission_owner_seat_id !== packet.from_agent) {
        throw new Error(
          `Unauthorized handover: "${packet.from_agent}" is not current mission_owner ("${mission.mission_owner_seat_id}")`
        );
      }

      // Increment fencing generation to invalidate previous holder
      const newFencingToken = await this.fencingClock.incrementGeneration(
        `mission_${missionId}`,
        packet.to_agent
      );
   ...
   */
  async executeUnplannedRecovery(
    missionId: string,
    controllerId: string,
    reason: string
    ...
    return await this.lockManager.withLock(`mission_${missionId}`, async () => {
      ...
      if (mission.studio_controller_seat_id !== controllerId) {
        throw new Error(
          `Unauthorized recovery: Seat "${controllerId}" is not registered studio_controller ("${mission.studio_controller_seat_id}")`
        );
      }
  ...
  async validateAction(
    ...
    const role: TwoSeatRole = isOwner ? 'mission_owner' : 'studio_controller';
   ...
   */
  async closeMission(missionId: string, ownerId: string, closeoutData: Record<string, unknown>): Promise<MissionRecord> {
    return await this.lockManager.withLock(`mission_${missionId}`, async () => {
      const mission = await this.getMission(missionId);
      if (!mission) {
        throw new Error(`Mission "${missionId}" not found`);
