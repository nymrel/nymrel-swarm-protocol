...
exports.DeliveryLedger = exports.DeliveryReceiptConflictError = exports.DELIVERY_REASON_CODES = exports.DELIVERY_EVIDENCE_KINDS = exports.DELIVERY_STATES = exports.DELIVERY_RECEIPT_VERSION = void 0;
const crypto = __importStar(require("node:crypto"));
const fs = __importStar(require("node:fs"));
const path = __importStar(require("node:path"));
const lock_1 = require("../fencing/lock");
exports.DELIVERY_RECEIPT_VERSION = '1.0';
...
exports.DELIVERY_EVIDENCE_KINDS = [
    'receipt_created',
    'outbox_persisted',
    'route_selected',
    'mailbox_persisted',
...
const STATE_SET = new Set(exports.DELIVERY_STATES);
const EVIDENCE_KIND_SET = new Set(exports.DELIVERY_EVIDENCE_KINDS);
const REASON_CODE_SET = new Set(exports.DELIVERY_REASON_CODES);
...
function normalizeTimestamp(at) {
    if (at === undefined)
...
function assertEvidence(evidence) {
    ...
    if (!candidate.kind || !EVIDENCE_KIND_SET.has(candidate.kind)) {
        throw new Error(`evidence.kind must be one of: ${exports.DELIVERY_EVIDENCE_KINDS.join(', ')}`);
    }
...
function assertReasonCode(reasonCode) {
    if (typeof reasonCode !== 'string' || !REASON_CODE_SET.has(reasonCode)) {
        throw new Error(`reason_code must be one of: ${exports.DELIVERY_REASON_CODES.join(', ')}`);
    }
}
function matchesEvidence(left, right) {
...
class DeliveryReceiptConflictError extends Error {
}
exports.DeliveryReceiptConflictError = DeliveryReceiptConflictError;
class DeliveryLedger {
    ...
    static computeTransitionHash(messageId, recipient, transition) {
        const material = [
            messageId,
            recipient,
    ...
    async create(params) {
        ...
        if (params.evidence !== undefined)
            assertEvidence(params.evidence);
        if (params.reason_code !== undefined)
            assertReasonCode(params.reason_code);
        ...
        return this.lockManager.withLock(lockName, async () => {
            ...
            if (existing) {
                ...
                const sameEvidence = creation.evidence?.kind === requestedEvidence?.kind &&
                    creation.evidence?.reference === requestedEvidence?.reference &&
                    creation.evidence?.sha256 === requestedEvidence?.sha256;
                const requestedAtMatches = params.at === undefined || creation.at === normalizeTimestamp(params.at);
                if (creation.actor !== params.actor ||
                    !sameEvidence ||
                    creation.reason_code !== params.reason_code ||
                    !requestedAtMatches) {
                    throw new DeliveryReceiptConflictError('Delivery receipt already exists with a different creation contract');
                }
            ...
            const receipt = {
                version: exports.DELIVERY_RECEIPT_VERSION,
    ...
    async transition(params) {
        ...
        return this.lockManager.withLock(lockName, async () => {
            ...
            if (!receipt) {
                throw new Error(`No delivery receipt exists for message "${params.message_id}" and recipient "${params.recipient}"`);
            }
            if (receipt.current_state === params.to) {
                ...
                if (!prior
                    ...
                    throw new Error(`Conflicting same-state retry for message "${params.message_id}" and recipient "${params.recipient}"`);
                }
            ...
            if (!DeliveryLedger.canTransition(receipt.current_state, params.to)) {
                throw new Error(`Invalid delivery transition ${receipt.current_state} -> ${params.to} for message "${params.message_id}"`);
            }
            const at = normalizeTimestamp(params.at);
    ...
    list(messageId) {
        ...
        return receipts.sort((left, right) => `${left.message_id}\u0000${left.recipient}`.localeCompare(`${right.message_id}\u0000${right.recipient}`));
    }
    static assertReceipt(receipt) {
        if (!receipt || typeof receipt !== 'object' || Array.isArray(receipt)) {
            throw new Error('Delivery receipt must be an object');
        }
        assertClosedObject(receipt, RECEIPT_KEYS, 'receipt');
        const candidate = receipt;
        if (candidate.version !== exports.DELIVERY_RECEIPT_VERSION) {
        ...
        for (let index = 0; index < candidate.transitions.length; index += 1) {
            const transition = candidate.transitions[index];
            ...
            if (transition.evidence !== undefined)
                assertEvidence(transition.evidence);
            if (transition.reason_code !== undefined)
                assertReasonCode(transition.reason_code);
            ...
            const expectedHash = DeliveryLedger.computeTransitionHash(candidate.message_id, candidate.recipient, draft);
    ...
    assertState(value) {
        if (typeof value !== 'string' || !STATE_SET.has(value)) {
            throw new Error(`Delivery state must be one of: ${exports.DELIVERY_STATES.join(', ')}`);
        }
    }
    digest(value) {
        return crypto.createHash('sha256').update(value, 'utf8').digest('hex');
    }
    getReceiptPath(messageId, recipient, createParent) {
        const dir = path.join(this.receiptsDir, this.digest(messageId));
        if (createParent)
            fs.mkdirSync(dir, { recursive: true });
        return path.join(dir, `${this.digest(recipient)}.json`);
    }
    getLockName(messageId, recipient) {
...
exports.DeliveryLedger = DeliveryLedger;
